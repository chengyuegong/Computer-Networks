/* Packet.java
 * Modified by: Wentao Wang
 * Date last modified: October 6
 *
 * Description: DHT Packet for DhtServer and DhtClient
 */
...
public class Packet {
    ...
    public boolean unpack(byte[] buf, int bufLen) {
        ...
        for (int i = 1; i < lines.length; i++) {
            ...
            if (left.equals("type")) {
            ...
            } else if (left.equals("key")) { // +key
                key = right;
            } else if (left.equals("val")) { // +val
                val = right;
            } else if (left.equals("reason")) { // +reason
                reason = right;
            ...
            } else if (left.equals("relayAdr")) { // +relayAdr
                chunks = right.split(":");
                if (chunks.length != 2) return false;
                relayAdr = new InetSocketAddress(chunks[0],
                        Integer.parseInt(chunks[1])); 
            } else if (left.equals("tag")) { // +tag
                tag = Integer.parseInt(right); 
            } else if (left.equals("hashRange")) { // +hashRange
                chunks = right.split(":");
                if (chunks.length != 2) return false;
                int leftRange = Integer.parseInt(chunks[0]);
                int rightRange = Integer.parseInt(chunks[1]);
                hashRange = new Pair<Integer,Integer>(
                                leftRange,rightRange);  
            } else if (left.equals("senderInfo")) { // +senderInfo
                chunks = right.split(":");
                ...
                succInfo = new
                    Pair<InetSocketAddress,Integer>(
                    new InetSocketAddress(ip,port),hash);
            } else if (left.equals("predInfo")) { // +predInfo
                chunks = right.split(":");
                if (chunks.length != 3) return false;
                String ip = chunks[0];
                int port = Integer.parseInt(chunks[1]);
                ...
                System.out.println("Type \"" + left + "\" doesn't match defined field.");
            }
        }
        return true;
    }
